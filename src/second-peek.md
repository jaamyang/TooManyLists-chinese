...
map方法取到了`self`的值，并且在接受一个输入后输出一个Option。之前的Ok的原因是知识将它传出去，但现在我们却想要离开它的作用域。*正确*的做法是通过`as_ref`方法来处理Option，它的定义如下：
...
这一步的操作使Option<T>内部的T变为一个引用。我们也可以自己通过显示的match来实现，但还是*算了吧*。使用`as_ref`意味着我们需要
